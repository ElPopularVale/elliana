...
uint8_t deviceIP[4] = { 192, 168, 0, 210 };
...
uint16_t ackTcp(TCPhdr* tcp, uint16_t len) {
	...
	tcp->ip.chksum =
			(uint16_t) HTONS(~(chksum(0,((uint8_t*)tcp) + sizeof(EtherNetII),
...
uint8_t GetPacket(uint8_t protocol, uint8_t* packet) {
	uint8_t i;
	//Should make this return 0 after so many failed packets
	for (i = 0; i < 255; ++i) {
		uint16_t len;
...
uint16_t IPstackHTMLPost(const uint8_t* url, const char * data, uint8_t* reply) {
	...
	uint16_t pseudochksum = chksum(TCPPROTOCOL + len - sizeof(IPhdr),
			tcp->ip.source, 8);
	tcp->chksum =
			(uint16_t) HTONS(~(chksum(pseudochksum,packet + sizeof(IPhdr), len-sizeof(IPhdr))));
	tcp->ip.chksum =
			(uint16_t) HTONS(~(chksum(0, packet+sizeof(EtherNetII), sizeof(IPhdr)-sizeof(EtherNetII))));
